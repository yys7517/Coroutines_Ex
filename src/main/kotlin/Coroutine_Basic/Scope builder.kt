...
// 자신만의 스코프를 선언할 수 있다.
// CoroutineScope - 이것은 코루틴의 범위를 만들고 모든 하위 코루틴들이 완료될 때까지 완료되지 않습니다.
...
// coroutineScope 의 경우는 자식 스레드가 완료될 때까지 현재 스레드를 block 하지 않고, 일시중단한다.
...
fun main() = runBlocking {
    ...
    coroutineScope{
        // CoroutineScope - 이것은 코루틴의 범위를 만들고 모든 하위 launch 들이 완료될 때까지 main 스레드를 일시중단.
        launch {
            delay(500L)
    ...
    // CoroutineScope 의 모든 하위 launch 들이 완료될 때까지 CoroutineScope 내의 실행이 완료되지 않기 때문에,
    // main 스레드는 CoroutineScope 의 완료까지 일시중단되므로 아래 구문은 마지막으로 동작한다.
