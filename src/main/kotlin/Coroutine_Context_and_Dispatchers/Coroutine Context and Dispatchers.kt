...
// Coroutine Context - Coroutines 는 실행이 될 때, CoroutineContext 에서 실행된다. "Coroutine이 실행되는 환경"

// 하지만, 우리는 앞서 배운 내용 중,
// launch 의 CoroutineContext parameter 로 CoroutineExceptionHadler 와 Dispatcher 가 들어가는 것을 볼 수 있다.
// 이것이 가능한 이유는 각 각 CoroutineContext 를 확장하는 인터페이스 구현체이기 때문이다.
...
/*
    ...
    * Dispatchers.Main - 이 디스패처를 사용하여 "메인 스레드"에서 코루틴을 실행합니다. (Android - UI Thread)
 ...
 */
// 모든 Coroutine 의 builder(launch, async, runBlocking, ...) 들은 CoroutineContext parameter 를 갖고 있다.
...
fun main() : Unit = runBlocking {
    ...
    launch {
        ...
        // 자신이 실행된 Context 를 상속받아서 작업하기 때문에, runBlocking 과 같은 Context 인 Main Thread 에서 실행하게 된다.
    ...
    newThreadDispatcher.use { myOwnThread ->
        launch(myOwnThread) {
            println("newSingleThreadContext: I'm working in thread ${Thread.currentThread().name}")
